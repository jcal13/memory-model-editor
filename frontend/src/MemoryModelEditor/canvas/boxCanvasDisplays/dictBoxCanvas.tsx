...
export default function DictBoxCanvas({ element, openDictInterface, updatePosition }: Props) {
  ...
  useEffect(() => {
    ...
    text.setAttribute("font-size", (10 * scale).toString());
    text.setAttribute("fill", "#333");
    text.textContent = "{}";
    gRef.current.appendChild(text);
  }, [element]);
  ...
  const outerW = 60 * 2 + 2 * (3 * 2);
  const outerH = 24 * 2 + 2 * (3 * 2);
  ...
  return (
    <g
      ...
      onClick={(e) => { e.stopPropagation(); openDictInterface(element); }}
