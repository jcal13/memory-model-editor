...
export default function ListBoxCanvas({ element, openListInterface, updatePosition }: Props) {
  ...
  const pad = 3 * scale;
  const totalW = mainWidth + 2 * pad;
  const totalH = mainHeight + 2 * pad;
  ...
  useEffect(() => {
    ...
    // Type box (top-right)
    const typeBoxX = mainX + mainWidth + 2 * scale - idBoxWidth + 1;
  ...
  // convert clientX/Y to SVG coordinates for dragging
  ...
  return (
    <g
      ...
      onClick={(e) => { e.stopPropagation(); openListInterface(element); }}
